# Service Architecture - agent-orchestration-benchmark
...
- **Lane:** Framework selection and benchmark governance
- **Primary buyer:** Engineering leads selecting agent frameworks
- **First motion:** Framework selection review with reproducible benchmark packet

## Recommended Architecture

Static report surface plus scheduled CI benchmark jobs; optional artifact bucket for report history.
...
- optional OpenAI/API provider keys for private benchmark runs

## Information Needed From Account Owner

- provider API keys in CI only when benchmark runs need them
...
- benchmark config committed with outputs
- no private prompts in public reports
